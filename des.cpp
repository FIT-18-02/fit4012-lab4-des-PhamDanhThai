...
string des_unit(string block, string key, bool encrypt) {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        string new_right = xor_strings(left, p);
        left = right;
        right = new_right;
    }
...
int main() {
    ...
    if (mode == 1 || mode == 2) {
        cin >> k1;
        cout << des_unit(data, k1, mode == 1) << endl;
    } 
    else if (mode == 3 || mode == 4) {
        cin >> k1 >> k2 >> k3;

        if (mode == 3) {
            string t = des_unit(data, k1, true);
            t = des_unit(t, k2, false);
            cout << des_unit(t, k3, true) << endl;
        } else {
            string t = des_unit(data, k3, false);
